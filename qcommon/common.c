...
void MSG_ReadDeltaUsercmd (sizebuf_t *msg_read, usercmd_t *from, usercmd_t *move)
{
	...
	memcpy_fast (move, from, sizeof(*move));
...
{
	memcpy_fast (SZ_GetSpace(buf,length),data,length);		
}
...
void SZ_Print (sizebuf_t *buf, char *data)
{
	...
	if (buf->cursize)
	{
		if (buf->data[buf->cursize-1])
			memcpy_fast ((byte *)SZ_GetSpace(buf, len),data,len); // no trailing 0
		else
			memcpy_fast ((byte *)SZ_GetSpace(buf, len-1)-1,data,len); // write over trailing 0
	}
	else
		memcpy_fast ((byte *)SZ_GetSpace(buf, len),data,len);
}
...
void Qcommon_Init (int argc, char **argv)
{
	...
	printf("After command/cvar/key init:\n");
	malloc_stats();
