...
// per-level limits
//
#define	MAX_CLIENTS			1		// absolute limit
#define	MAX_EDICTS			900 // must change protocol to increase more     Bruce this needs looking into. 
...
#ifdef __DREAMCAST__
    #include <dc/fmath.h>
#define sin(x) shz_sinf((float)(x))
#define cos(x) shz_cosf((float)(x))
#define tan(x) shz_tanf((float)(x))
#define sqrt(x) shz_sqrtf((float)(x))
    // Don't redefine sqrt - use fsqrt directly where needed
#endif
...
#include <dc/perfctr.h>
 ...
 static inline void prof_start1(void) {
    perf_cntr_clear(PRFC1);
    perf_cntr_start(PRFC1, PMCR_ELAPSED_TIME_MODE, PMCR_COUNT_CPU_CYCLES);
}

static inline void prof_end1(const char *label) {
    perf_cntr_stop(PRFC1);
    uint64_t cycles = perf_cntr_count(PRFC1);
    printf("%s: %llu cycles (%.2f us)\n", label, cycles, cycles * 0.005);  // 5ns per cycle at 200MHz
}


static inline void prof_end_threshold(const char *label, int threshold) {
    perf_cntr_stop(PRFC1);
    uint64_t cycles = perf_cntr_count(PRFC1);
    if (cycles > threshold)
        printf("%s: %llu cycles (%.2f us)\n", label, cycles, cycles * 0.005);
}

#define sin(x) ({ prof_start1(); float _r = sin(x); prof_end_threshold("sin", 2200); _r; })
#define cos(x) ({ prof_start1(); float _r = cos(x); prof_end_threshold("cos", 2200); _r; })
#define tan(x) ({ prof_start1(); float _r = tan(x); prof_end_threshold("tan", 2200); _r; })
...
#define DotProduct(x,y) fipr(x[0], x[1], x[2], 0.0f, y[0], y[1], y[2], 0.0f) // Bruce
...
extern float monster_flash_offset[][3];
...
typedef enum
{
	QEV_NONE,   // kos conflict with EV_NONE  with master. 
	EV_ITEM_RESPAWN,
	EV_FOOTSTEP,
	EV_FALLSHORT,
	EV_FALL,
