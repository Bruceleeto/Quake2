...
#ifdef __DREAMCAST__
    if (ps->sfx && ps->sfx->cache == (sfxcache_t *)0x1) {
        S_PlayADPCM(ps->sfx->name);
        S_FreePlaysound(ps);
        return;
    }
    // Don't process non-ADPCM sounds on Dreamcast
    S_FreePlaysound(ps);
...
struct sfx_s *S_RegisterSexedSound (entity_state_t *ent, char *base)
{
	...
	// Force male sounds only - skip all the gender detection and fallback logic
	// Just go straight to male sounds to save memory and processing time
	
	Com_sprintf (maleFilename, sizeof(maleFilename), "player/male/%s", base+1);
	sfx = S_RegisterSound (maleFilename);
...
void S_Update_(void)
{
	...
	if (!sound_started)
		...
		#ifdef __DREAMCAST__
    // Process pending sounds directly on Dreamcast
    playsound_t *ps;
    while ((ps = s_pendingplays.next) != &s_pendingplays)
    {
        if (ps->sfx && ps->sfx->cache == (sfxcache_t *)0x1) {
            S_PlayADPCM(ps->sfx->name);
        }
