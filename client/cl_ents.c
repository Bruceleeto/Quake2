...
struct model_s *S_RegisterSexedModel (entity_state_t *ent, char *base)
{
	...
	// Force male model for memory savings - skip all the gender detection
	// and fallback logic since we're only using male models anyway
	
	Com_sprintf (buffer, sizeof(buffer), "players/male/%s", base+1);
	mdl = re.RegisterModel(buffer);
	if (!mdl) {
		// fallback to default male weapon model
...
void CL_AddViewWeapon (player_state_t *ps, player_state_t *ops)
{
	...
	int			i;
	// Bruce Loook into this properly  related to weapon issue.
